...
CREATE TABLE `orders` (
  ...
  `order_number` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '订单编号，用户友好的订单展示编号，具有业务可读性',
  ...
  `currency` varchar(20) COLLATE utf8mb4_bin NOT NULL DEFAULT 'USDT' COMMENT '支付货币类型，默认USDT，支持多种数字货币支付',
  `blockchain_type` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '区块链网络类型，如TRC20、ERC20、BEP20等，用于区分不同网络支付',
  `blockchain_address` varchar(255) COLLATE utf8mb4_bin NOT NULL COMMENT '支付接收地址，用户需要向此地址转账支付订单',
  `transaction_hash` varchar(128) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '区块链交易哈希，用于验证支付完成状态，支付成功后填写',
  `is_paid` tinyint(1) NOT NULL DEFAULT '0' COMMENT '支付状态标识，true=已支付，false=待支付',
  `status` enum('pending','paid','completed','cancelled','expired') COLLATE utf8mb4_bin NOT NULL DEFAULT 'pending' COMMENT '订单状态：pending待支付/paid已支付/completed已完成/cancelled已取消/expired已过期',
...
CREATE TABLE `power_packages` (
  ...
  `name` varchar(100) COLLATE utf8mb4_bin NOT NULL COMMENT '算力包名称，如"基础算力包"、"高级算力包"等',
  `task_type` varchar(100) COLLATE utf8mb4_bin NOT NULL COMMENT '任务类型，如"AI智能计算"、"数据处理"等',
  `required_level` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '购买所需的最低用户等级，0-5对应LV.0-LV.5',
  `earnings_percent` decimal(10,4) NOT NULL COMMENT '收益率百分比，表示每日收益的百分比，如0.5表示日收益率0.5%',
  `amount` decimal(20,8) NOT NULL COMMENT '算力包价格，8位小数精度，用户需要支付的费用',
  `currency` varchar(20) COLLATE utf8mb4_bin NOT NULL DEFAULT 'USDT' COMMENT '价格货币单位，默认USDT，指定支付货币类型',
  `duration_days` int unsigned NOT NULL COMMENT '算力有效期天数，如30天、90天等，算力服务的有效期限',
  `description` text COLLATE utf8mb4_bin NOT NULL COMMENT '算力包详细描述，包含服务内容和收益说明，帮助用户了解产品',
  `features` json DEFAULT NULL COMMENT '算力包特性列表JSON，如专属客服、优先支持等，产品特色功能',
  `daily_earnings` decimal(20,8) NOT NULL COMMENT '预计每日收益金额，8位小数精度，基于收益率计算的理论收益',
  `total_earnings` decimal(20,8) NOT NULL COMMENT '预计总收益金额，8位小数精度，整个有效期的理论总收益',
  `is_active` tinyint(1) NOT NULL DEFAULT '1' COMMENT '算力包状态，true=在售，false=停售',
...
CREATE TABLE `system_configs` (
  ...
  `config_type` enum('string','number','boolean','json') COLLATE utf8mb4_bin NOT NULL DEFAULT 'string' COMMENT '配置类型：string字符串/number数字/boolean布尔值/json对象',
  `description` text COLLATE utf8mb4_bin COMMENT '配置描述，详细说明该配置的用途和含义，null表示无说明',
  `is_encrypted` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否加密，true=敏感信息需要加密存储，false=明文存储',
...
CREATE TABLE `task_configs` (
  ...
  `name` varchar(200) COLLATE utf8mb4_bin NOT NULL COMMENT '任务名称，用户友好的任务展示名称',
  `task_type` enum('text_training','table_training','image_training','video_training') COLLATE utf8mb4_bin NOT NULL COMMENT '任务类型：text_training文本训练/table_training表格训练/image_training图像训练/video_training视频训练',
  ...
  `currency` varchar(20) COLLATE utf8mb4_bin NOT NULL DEFAULT 'USDT' COMMENT '奖励货币类型，默认USDT',
  `description` text COLLATE utf8mb4_bin NOT NULL COMMENT '任务详细描述，说明任务内容、要求和注意事项',
  `difficulty` enum('easy','medium','hard') COLLATE utf8mb4_bin NOT NULL DEFAULT 'easy' COMMENT '任务难度：easy简单/medium中等/hard困难，影响奖励金额和完成时间',
  `completion_time` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '预计完成时间，如"2-3小时"，给用户的参考时间',
  `is_active` tinyint(1) NOT NULL DEFAULT '1' COMMENT '任务状态，true=可用，false=停用，停用任务不对外显示',
...
CREATE TABLE `transactions` (
  ...
  `type` enum('deposit','withdraw','exchange','purchase','airdrop','referral','task_earning','mining_earning') COLLATE utf8mb4_bin NOT NULL COMMENT '交易类型：充值/提现/兑换/购买/空投/邀请/任务收益/挖矿收益',
  `from_currency` varchar(20) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '源货币代码，兑换交易的转出货币，如BTC、USDT等',
  `to_currency` varchar(20) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '目标货币代码，兑换交易的转入货币，如ETH、DG等',
  ...
  `exchange_rate` decimal(20,8) DEFAULT NULL COMMENT '兑换汇率，用于币种兑换交易，表示两个货币之间的兑换比例',
  ...
  `blockchain_type` varchar(20) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '区块链类型，如TRC20、ERC20等，用于链上交易',
  `transaction_hash` varchar(128) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '区块链交易哈希，用于追踪链上交易状态',
  `from_address` varchar(255) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '转出地址，充值或提现的区块链钱包地址',
  `to_address` varchar(255) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '转入地址，充值或提现的区块链钱包地址',
  `confirmations` int unsigned DEFAULT NULL COMMENT '区块链确认数，表示交易被区块确认的次数，越多越安全',
  `description` text COLLATE utf8mb4_bin COMMENT '交易描述，用户友好的交易说明',
...
CREATE TABLE `user_power_records` (
  ...
  `order_id` bigint unsigned DEFAULT NULL COMMENT '关联的订单ID，外键关联orders表，null表示非订单购买',
  `type` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '算力类型，继承自算力包的task_type字段',
  `amount` decimal(20,8) NOT NULL COMMENT '算力包价格，8位小数精度',
  `currency` varchar(20) COLLATE utf8mb4_bin NOT NULL DEFAULT 'USDT' COMMENT '价格货币单位，默认USDT',
  `start_time` timestamp NOT NULL COMMENT '算力开始生效时间',
  `end_time` timestamp NOT NULL COMMENT '算力到期时间',
  `status` enum('active','expired','cancelled') COLLATE utf8mb4_bin NOT NULL DEFAULT 'active' COMMENT '算力状态：生效中/已过期/已取消',
  `earnings` decimal(20,8) NOT NULL DEFAULT '0.00000000' COMMENT '累计收益金额，8位小数精度',
  `current_hashrate` decimal(20,8) DEFAULT NULL COMMENT '当前算力值，8位小数精度，单位TH/s',
  `total_hashrate` decimal(20,8) DEFAULT NULL COMMENT '总算力值，8位小数精度，单位TH/s',
...
CREATE TABLE `user_security_questions` (
  ...
  `answer_salt` varchar(255) COLLATE utf8mb4_bin NOT NULL COMMENT '答案盐值，用于增强答案安全性',
...
CREATE TABLE `users` (
  ...
  `inviter_id` bigint unsigned DEFAULT NULL COMMENT '邀请人的用户ID，外键关联users表',
